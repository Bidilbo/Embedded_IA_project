...
void RognerBitmapMax(BMP* pBitmap,unsigned char valMax){
  ...
  int offsetHaut=0;
  int offsetBas=0;
  int margeGauche=offsetGauche;
  int margeHaute=offsetHaut;
  int margeDroite=pBitmap->infoHeader.largeur-offsetDroit;
  int margeBasse=pBitmap->infoHeader.hauteur-offsetBas;
  int continu=1;
  int nouvelleHauteur;
  int nouvelleLargeur;
  ...
  continu=1;
  j=offsetGauche;
  while(continu && j<pBitmap->infoHeader.largeur-offsetDroit){
    for(i=offsetHaut;i<pBitmap->infoHeader.hauteur-offsetBas;i++){
      if(pBitmap->mPixelsGray[i][j]>valMax)
        continu=0;
    }
    j++;
  }
  if(j<pBitmap->infoHeader.largeur-1-offsetDroit)
    margeGauche=j-1;

  continu=1;
  j=pBitmap->infoHeader.largeur-1-offsetDroit;
  while(continu && j>=offsetGauche){
    for(i=offsetHaut;i<pBitmap->infoHeader.hauteur-offsetBas;i++){
      if(pBitmap->mPixelsGray[i][j]>valMax)
        continu=0;
      }
  ...
  continu=1;
  i=offsetHaut;
  while(continu && i<pBitmap->infoHeader.hauteur-offsetBas){
    for(j=offsetGauche;j<pBitmap->infoHeader.largeur-offsetDroit;j++){
      if(pBitmap->mPixelsGray[i][j]>valMax)
        continu=0;
      }
    i++;
  }
  if(i<pBitmap->infoHeader.hauteur-1-offsetBas)
    margeHaute=i-1;

  continu=1;
  i=pBitmap->infoHeader.hauteur-1-offsetBas;
  while(continu && i>=offsetHaut){
    for(j=offsetGauche;j<pBitmap->infoHeader.largeur-offsetDroit;j++){
      if(pBitmap->mPixelsGray[i][j]>valMax)
        continu=0;
      }
